...
if nargin<5
    trainerpath = [pwd '\']; %if no directory is specified for training program, then assume it is the current directory
end
...
status = system([trainerpath 'lstm_sequence ' trainerpath]); %execute the LSTM trainer from the Windows command prompt
...
status = system([trainerpath 'lstm_inference ' trainerpath]); %execute the LSTM tester from the Windows command prompt
