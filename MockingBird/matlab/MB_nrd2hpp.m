function [ data, timestamps ] = MB_bandpassHPP( fname, outname, channelsRequested, DS )


% ARGUMENTS
% fname = name of input nrd file to process (e.g., '/CheetahData/RawDataFile.nrd')
% fname = base filename of hpp file to write (channel number will be appended to each file written, e.g., 'datafile_ch23.HPP')
% channelsRequested = vector of data channels to store to HPP files

channelsRequested = channelsRequested + 1; % NLX CHANNELS ARE 0 BASED WHILE THE REQUEST IN MATLAB IS 1 BASED
...
%% OPEN THE FILEThe Matlab code is a correct implementation of the Neuralynx algorithm. (Neuralynx's document contains an error -- one cannot perform an OR to confirm a data summary, it needs to be an XOR.) The Matlab code is not a pretty implementation of the Neuralynx algorithm, as it's not even properly a function.
...
tt = regexp( strcat(header), 'SamplingFrequency\s+(?<SamplingFrequency>[0-9]+)\s+', 'names' );
baserate=str2num(tt.SamplingFrequency);
...
% save 'nrdData.mat' data -v7.3; %use -v7.3 switch for large files
