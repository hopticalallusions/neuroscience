...
% these current coefficients are based on minimum order FIR -- 146 coeffs
b_lowpass_fir = Num;
[row, N_coeffs] = size(b_lowpass_fir);

% x_lowpass should be the input data, y_lowpass filtered output
x_lowpass = lfp; % currently reading data from workspace
[row, N_samples] = size(lfp);
y_lowpass_fir = zeros(1, N_samples);

tic
for i=1:N_samples
    % difference equation
    for k=1:min(i,N_coeffs)
        y_lowpass_fir(i) = y_lowpass_fir(i) + x_lowpass(i-k+1)*b_lowpass_fir(k);
    end
end
toc
...
y_lowpass_iir = zeros(1, N_samples);
[row, N_coeffs] = size(b_lowpass_iir);

tic
% filter coefficients currently read from workspace
for i=1:N_samples
   % difference equation 
   for k=1:min(i,N_coeffs) 
        y_lowpass_iir(i) = y_lowpass_iir(i) ...
            - y_lowpass_iir(i-k+1)*a_lowpass_iir(k)...
            + x_lowpass(i-k+1)*b_lowpass_iir(k);
   end
end
toc
...
decimated_lfp = zeros(1, ceil(N_samples/decimationFactor));

tic
k = 1;
for i=1:N_samples
    if (mod(i-1, decimationFactor) == 0)
        decimated_lfp(k) = y_lowpass_iir(i);
        k = k + 1;
    end 
end
toc
...
for i=1:N_samples
   for k=1:min(i, 5)
        y(i) = y(i) - a(k)*y(i-k+1) + b(k)*decimatedLFP(i-k+1);
   end
end
toc
