...
#define DOWNSAMPLED_SIZE 2048 //((DATA_SIZE % DOWNSAMPLE_FACTOR == 0) ? (DATA_SIZE/DOWNSAMPLE_FACTOR) : (DATA_SIZE/DOWNSAMPLE_FACTOR+1))
...
void realtime_theta_phase_kernel( int *lfp, double *digitized);
...
void realtime_theta_phase_sw( int *lfp, double *digitized) {
	...
	double arctanLookup[CORDIC_PRECISION] = { 
	...
	for ( dataIndex=0; dataIndex < DATA_SIZE; dataIndex++ ) {
		//
		// LOW PASS FILTER
		for ( k=N_LOWPASS_COEFFS-1; k>0; k-- ) {
		...
		if ( 0 == dataIndex % DOWNSAMPLE_FACTOR ) {
			...
			for ( freqBandIdx = 0; freqBandIdx < N_BANKS; freqBandIdx++ ) {
				...
				for ( k=HILBERT_CACHE_DEPTH-1; k>0; k-- ) {
					hilbertCache[freqBandIdx][k]=hilbertCache[freqBandIdx][k-1];
				}
				hilbertCache[freqBandIdx][0]=bandPassOut;
				//if (freqBandIdx==16) { for ( k = 0; k<15; k++ ) { printf("%f\t",hilbertCache[freqBandIdx*N_BANKS+k]); } printf("\n"); for ( k = 0; k<15; k++ ) { printf("%d %d\t",freqBandIdx,k);} printf("\n"); for ( k = 0; k<15; k++ ) { printf("%d \t",freqBandIdx*N_BANKS+k);} }
				...
				for ( k=0; k < CORDIC_PRECISION ; k++ ) {
					...
					cordicYLast = cordicY;
					if ( cordicY < 0 ) {
						cordicX = cordicX - ( cordicYLast / (1<<k) );
						cordicY = cordicY + ( cordicXLast / (1<<k) );
...
int main() {
    ...
    */
	...
	for (int d=0; d<DOWNSAMPLED_SIZE;d++){ digitized[d]=0.0;}
    ...
    if (!(dfp = fopen("digitized_fpga.dat", "wa"))) {
        printf("File can not be open for read.\n");
        return -1;
    }
	elementsRetrieved = fread( lfp, 4, DATA_SIZE, elfp );
	gettimeofday( &timevalStart, NULL );
	#pragma ACCEL task
	realtime_theta_phase_kernel( lfp, digitized );
	gettimeofday( &timevalEnd, NULL );
	printf("%ld seconds, %d usec\n",  timevalEnd.tv_sec-timevalStart.tv_sec, timevalEnd.tv_usec-timevalStart.tv_usec);
	fclose( elfp );
	fwrite( digitized, sizeof(digitized[0]), sizeof(digitized)/sizeof(digitized[0]), dfp );
