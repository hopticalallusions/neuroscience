function [ avgLfp, lfpTimestamps ] = avgLfpFromList(dir, filelist, lfpStartIdx)
...
%     end
    ...
    for ii=1:length(filelist)
        if strcmp(filelist{ii}(end-3:end), '.ncs')
            disp([ 'loading ' dir filelist{ii} ]);
            if ii == 1
                [ temp, lfpTimestamps ] = csc2mat([ dir filelist{ii} ], lfpStartIdx);
            else
        ...
        elseif strcmp(filelist{ii}(end-3:end), '.dat')
            ...
            if ii == 1
                loadCExtractedNrdTimestampData([ dir 'timestamps.dat' ]);
            end
