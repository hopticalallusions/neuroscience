...
%[data]=csc2mat('/Volumes/SILVRSURFER/da5/2016-08-17/CSC44.ncs');
%[data]=csc2mat('/Volumes/SILVRSURFER/da5/2016-08-24-day1-rewards-plusmaze/CSC24.ncs');
%[data]=csc2mat('/Volumes/SILVRSURFER/da5/day6_plusmaze-rewarded_training3_8x_2016-08-27/CSC28.ncs');
%[events]=nev2mat('/Volumes/SILVRSURFER/da5/day6_plusmaze-rewarded_training3_8x_2016-08-27/Events.nev');
...
secStart = 638; secEnd = 640;

secStart = 3528; secEnd = 3530;  % moving nicely

secStart = 4900; secEnd = 4902;  % intense sharp wave
%secStart = 5014; secEnd = 5018;
sst=data(round(secStart*32000):round(secEnd*32000));
...
n = length(sst);
dt = 1/32000 ; % sampling resolution
time = [0:length(sst)-1]*dt ;  % construct time array
xlim = [0,length(sst)*dt];  % plotting range
pad = 1;      % pad the time series with zeroes (recommended)
dj = 1/64;    % specify sub octaves; 1/4 will do 4 sub-octaves per octave
...
%         plot."
%s0 = 1/2^13; % 64*dt; "smallest scale of the wavelet."
s0 = 1/(2^ceil(log2(maximumHz)));  %1/2^14;
...
j1 = ceil(log2(maximumHz)-log2(minimumHz))/dj;    % this says do <n>/dj powers-of-two with 1/dj sub-octaves each
...
colormap(build_NOAA_colorgradient(256));
if doNormalization
    thedims=size(power);
    ...
    imagesc( time,log2(period), normpower );
    caxis([0 1]);
else
