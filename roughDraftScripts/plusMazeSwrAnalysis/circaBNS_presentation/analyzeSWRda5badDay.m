function output = analyzeSWRda5badDay( metadata )
...
% crazy pants errors keep occurring on aug 30
[ blob.lfpTimestamps, idxSorted ] = sort(blob.lfpTimestamps);
blob.swrLfp = blob.swrLfp(idxSorted);
avgLfp = avgLfp(idxSorted);
avgDisconnectedLfp = avgDisconnectedLfp(idxSorted);

[ blob.lfpTimestamps, idxUnique, ~ ] = unique(blob.lfpTimestamps);
blob.swrLfp = blob.swrLfp(idxUnique);
avgLfp = avgLfp(idxUnique);
avgDisconnectedLfp = avgDisconnectedLfp(idxUnique);
