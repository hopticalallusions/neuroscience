...
% *** this might not work entirely smoothly. BE SURE TO CHECK THINGS if re
% run from scratch.
for ii=1:9
    [agg.(recDays{ii}).xpos, agg.(recDays{ii}).ypos]=rotateXYPositions( agg.(recDays{ii}).xpos, agg.(recDays{ii}).ypos, 412, 408, -140, 350, 350 ); 
    [agg.(recDays{ii}).swrXpos, agg.(recDays{ii}).swrYpos]=rotateXYPositions( agg.(recDays{ii}).swrXpos, agg.(recDays{ii}).swrYpos, 412, 408, -140, 350, 350 ); 
end
for ii=10:length(recDays)  % the camera moves.
    [agg.(recDays{ii}).xpos, agg.(recDays{ii}).ypos]=rotateXYPositions( agg.(recDays{ii}).xpos, agg.(recDays{ii}).ypos, 330, 241, -140, 350, 350 ); 
    [agg.(recDays{ii}).swrXpos, agg.(recDays{ii}).swrYpos]=rotateXYPositions( agg.(recDays{ii}).swrXpos, agg.(recDays{ii}).swrYpos, 330, 241, -140, 350, 350 ); 
end
%ii=10; plot( agg.(recDays{ii}).xpos, agg.(recDays{ii}).ypos ); hold on; scatter( agg.(recDays{ii}).swrXpos, agg.(recDays{ii}).swrYpos );

ii=10;
%agg.(recDays{ii}).ypos = agg.(recDays{ii}).ypos+356;
agg.(recDays{ii}).xpos = agg.(recDays{ii}).xpos-5;
%agg.(recDays{ii}).swrYpos = agg.(recDays{ii}).swrYpos+356;
agg.(recDays{ii}).swrXpos = agg.(recDays{ii}).swrXpos-5;




% for ii=1:9
%     agg.(recDays{ii}).xpos = agg.(recDays{ii}).xpos - (412 - 349 );
%     agg.(recDays{ii}).ypos = agg.(recDays{ii}).ypos - (408 - 348 );
%     agg.(recDays{ii}).swrXpos = agg.(recDays{ii}).swrXpos - (412 - 349 );
%     agg.(recDays{ii}).swrYpos = agg.(recDays{ii}).swrYpos - (408 - 348 );
% end
% for ii=1:9
%     agg.(recDays{ii}).xpos = agg.(recDays{ii}).xpos + 150;
%     agg.(recDays{ii}).swrXpos = agg.(recDays{ii}).swrXpos + 150;
%     agg.(recDays{ii}).ypos = agg.(recDays{ii}).ypos - 106;
%     agg.(recDays{ii}).swrYpos = agg.(recDays{ii}).swrYpos -106;
% end
...
%ii=5; figure; plot( agg.(recDays{ii}).xpos, agg.(recDays{ii}).ypos ); hold on; scatter( agg.(recDays{ii}).swrXpos, agg.(recDays{ii}).swrYpos );
...
% Tad requested that I make this custom for each arm to compensate for the
% camera distortion
% North 317 px
% East  340 px
% South 340 px
...
for ii=1:length(recDays)
    ...
    [angle, radius] = cart2pol( agg.(recDays{ii}).xpos-310, agg.(recDays{ii}).ypos-533 ); angle=angle*180/pi+180;
    rad=radius;
    agg.(recDays{ii}).xylinearized(find((rad <= 25))) =  120; % Center Point                                                                                     px / cm  + offset
    agg.(recDays{ii}).xylinearized(find((rad > 25) .* ( angle >  45 ) .* ( angle <= 135 ))) =      rad(find((rad > 25) .* ( angle >  45 ) .* ( angle <= 135 )))/(315/100) + 225; % South
    agg.(recDays{ii}).xylinearized(find((rad > 25) .* ( angle > 135 ) .* ( angle <= 225 ))) =      rad(find((rad > 25) .* ( angle > 135 ) .* ( angle <= 225 )))/(300/100) + 330; % East
    agg.(recDays{ii}).xylinearized(find((rad > 25) .* ( angle > 225 ) .* ( angle <= 315 ))) =      rad(find((rad > 25) .* ( angle > 225 ) .* ( angle <= 315 )))/(310/106) + 125; % North  ; invert this so the rat starts at x=0
    agg.(recDays{ii}).xylinearized(find((rad > 25) .* ( angle > 315 )  + ( angle <=  45 ))) = 115-(rad(find((rad > 25) .* ( angle > 315 )  + ( angle <=  45 )))/(275/106)) ; % West
    ...
    idxs=[]; for jj=1:length(agg.(recDays{ii}).swrTimes); idxs=[ idxs find( (agg.(recDays{ii}).swrTimes(jj) < agg.(recDays{ii}).xytimestampSeconds) , 1 )]; end; 
...
%         end
       ...
       % endIdx = startIdx + (20*30);
        ...
        perTrial(trialIdx).isSubTrial       = 0;% agg.(recDays{ii}).isSubTrial(jj);
        ...
        perTrial(trialIdx).beeline          = 0; % agg.(recDays{ii}).beeline(jj);
...
hold on;
for ii=1:length(perTrial)
    scatter( perTrial(ii).xylinearized./pxPerCm,  ones(size(perTrial(ii).xylinearized))*ii, 'o', 'filled'); alpha(.01);
end; xlim([0 125*4]); set(gca, 'Ydir', 'reverse'); axis tight;
title('occupancy')
subplot(7,4,2:4:6*4); %imagesc(swrAll);  
hold on;
for ii=1:length(perTrial)
    scatter( perTrial(ii).swrLinearized./pxPerCm,  ones(size(perTrial(ii).swrLinearized))*ii, 'o', 'filled'); alpha(.2);
end;xlim([0 125*4]); set(gca, 'Ydir', 'reverse')
