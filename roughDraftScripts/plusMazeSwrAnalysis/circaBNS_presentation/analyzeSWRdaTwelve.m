function output = analyzeSWRdaTwelve( metadata )
...
[ blob.swrLfp ] = loadCExtractedNrdChannelData( [ metadata.dir metadata.swrLfpFile ] );
blob.lfpTimestamps = loadCExtractedNrdTimestampData( [ metadata.dir '/timestamps.dat' ]);
...
% fix any non-monotonicity
% this is not the right place/way to do this, but we're going to do it anyway.
[ blob.lfpTimestamps, mapping ] = sort(blob.lfpTimestamps);
blob.lfpTimestampSeconds = blob.lfpTimestampSeconds(mapping);
avgLfp = avgLfp(mapping);
blob.swrLfp = blob.swrLfp(mapping);

negIdx=find(diff(blob.lfpTimestamps)<=0);
warning([ 'replacing ' num2str(length(negIdx)) ' redundant elements' ]);
blob.lfpTimestamps(negIdx) = [];
blob.lfpTimestampSeconds(negIdx) = [];
avgLfp(negIdx) = [];
blob.swrLfp(negIdx) = []; 
...
%
electricLFP=filtfilt( filters.so.electric, avgLfp );
...
% ==================================================
proxToCenter=proxToPoint( blob.xpos, blob.ypos, 315, 219 );
proxToStart=proxToPoint( blob.xpos, blob.ypos, 503, 59 );
proxToRewardSite=proxToPoint( blob.xpos, blob.ypos, 118, 47 );
proxToIncorrectSite=proxToPoint( blob.xpos, blob.ypos, 518, 428 );
%
% if metadata.visualizeAll
...
plot( mvgMedianTimes, mvgMedian ); 
hold on; %plot( mvgMedianTimes(2:end), -diff(mvgMedian) ); 
plot( xyblob.lfpTimestampSeconds, proxToRewardSite );
scatter( output.ontoMazeTimes, ones(1,length(output.ontoMazeTimes)).*0.1, 'v', 'MarkerEdgeColor', [ .5 .4 0 ], 'MarkerFaceColor', [ 1 .8 0 ] );
scatter( output.runStartTimes, ones(1,length(output.runStartTimes)).*0.1, '>',  'MarkerEdgeColor', [ .2 .6 0 ], 'MarkerFaceColor', [ .2 .6 0 ]);
scatter( output.rewardTimes, ones(1,length(output.rewardTimes)).*0.1, 'o', 'filled', 'MarkerEdgeColor', [ .4 .4 .9 ], 'MarkerFaceColor', [ .4 .4 .9 ]);
scatter( output.intoBucketTimes, ones(1,length(output.intoBucketTimes)).*0.1, '^', 'filled', 'MarkerEdgeColor', [ .9 .1 .1 ], 'MarkerFaceColor', [ .9 .1 .1 ]);
ylabel( 'start prox./trials' ); axis tight; ylim([ 0 1 ]);
print( gcf(4), [metadata.outputDir metadata.rat '_' metadata.day  '_' metadata.swrLfpFile '_finalAutoTrialMarking'],'-dpng','-r200');
...
print( gcf(5), [metadata.outputDir metadata.rat '_' metadata.day '_' metadata.swrLfpFile '_SWRrateSummary'],'-dpng','-r200');

save([metadata.outputDir metadata.rat '_' metadata.day '_' metadata.swrLfpFile '_output'], 'output' );
