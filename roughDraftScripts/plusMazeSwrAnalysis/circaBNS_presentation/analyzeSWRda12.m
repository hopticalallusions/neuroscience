function output = analyzeSWRda12( metadata )
...
%rawTemp = loadCExtractedNrdChannelData('/Volumes/Seagate Expansion Drive/plusmaze-habit/da12/2017-10-24_orientation2/rawChannel_56.dat' );
avoidStupidMatlabError = [ metadata.dir metadata.swrLfpFile ];
rawTemp = loadCExtractedNrdChannelData(avoidStupidMatlabError);
%blob.swrLfp = filtfilt( DCO, rawTemp );
dcoComponent = conv( rawTemp, ones(8000,1)/8000, 'same');
blob.swrLfp = rawTemp - dcoComponent ;
blob.lfpTimestamps = loadCExtractedNrdTimestampData([ '/Volumes/Seagate Expansion Drive/plusmaze-habit/' metadata.rat '/' metadata.day '/' 'timestamps.dat' ]);
blob.lfpTimestampSeconds=(blob.lfpTimestamps-blob.lfpTimestamps(1))/1e6;


%% These next lines are required for raw data, to ensure a monotonic input to the peakfind routines.
toc
disp('fix temporal order');
%% fix temporal order
disp([ 'lfpTimestamps   ' num2str(length(blob.lfpTimestamps))  '   lfpTimestampSecs   '  num2str(length(blob.lfpTimestampSeconds)) '   swrLfp   '  num2str(length(blob.swrLfp))  '   avgLfp   ' num2str(length(avgLfp))  '   avgDiscLfp   ' num2str(length(avgDisconnectedLfp))  ])
[ blob.lfpTimestamps, reorderedIdxList ] = sort(blob.lfpTimestamps);
disp([ 'reorderedLength   ' num2str(length(reorderedIdxList)) ])
toc
blob.lfpTimestampSeconds = blob.lfpTimestampSeconds(reorderedIdxList);
toc
blob.swrLfp = blob.swrLfp (reorderedIdxList);
toc
avgLfp = avgLfp (reorderedIdxList);
toc
avgDisconnectedLfp = avgDisconnectedLfp (reorderedIdxList);
toc
%% de-dupetoc
disp('de-duping');
disp([ 'starting with ' num2str(length(blob.lfpTimestamps)) ]);
[ blob.lfpTimestamps, uniqueIdxs, rebuildIdxs ] = unique(blob.lfpTimestamps);
blob.lfpTimestampSeconds = blob.lfpTimestampSeconds(uniqueIdxs);
disp([ 'uniqueLength   ' num2str(length(uniqueIdxs)) ])
toc
blob.swrLfp = blob.swrLfp (uniqueIdxs);
toc
avgLfp = avgLfp (uniqueIdxs);
toc
avgDisconnectedLfp = avgDisconnectedLfp (uniqueIdxs);
toc


% figure; plot( blob.lfpTimestampSeconds, rawTemp ); hold on; 
% rr=conv( rawTemp, ones(8000,1)/8000, 'same');
% plot( blob.lfpTimestampSeconds, rr )
...
%         [ ~, jj ] = min( abs( relativeTimes ));
%         offset = 0;
% %         disp( num2str( jj+offset < length(relativeTimes) ) )
% %         disp( num2str( relativeTimes(jj+offset) < 40 ) )
...
[ swrPeakValues,      ...
  ...
  swrPeakWidths ] = findpeaks( swrblob.swrLfpEnv,                        ... % data
                             ...
                             'MinPeakHeight',  swrThreshold, ... % prctile( swrLfpEnvelope, percentile ), ... % default 95th percentile peak height
