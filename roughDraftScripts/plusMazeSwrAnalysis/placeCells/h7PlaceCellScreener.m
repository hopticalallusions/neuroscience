...
dir='/Volumes/AHOWETHESIS/h7/2018-09-06_16-05-49/';
...
dateStr='2018-09-06';
startCoordinate = [ 250, 50 ]; startRadius = 32;%  {130,280}  start area  {radius}
bucketCoordinate =[ 250,350 ]; bucketRadius = [  47 ];
...
print(gcf(1), [ '~/data/h7_positionPlot1_' dateStr '_.png'],'-dpng','-r200');
...
% I have no idea what this should be set to...
% tt13, tt19 - theta   ch84  maybe others not noted.
[ lfpTheta, lfpTimestamps ]=csc2mat( [ dir 'CSC84.ncs' ] );
...
print(gcf(1), [ '~/data/h7_positionPlot_' dateStr '_.png'],'-dpng','-r200');
...
for ttIdx=1:length(ttFilenames)
    ...
    for whichCell=0:max(cellNFast)
        if sum(cellNFast==whichCell) > 2
            ...
            %
            print(gcf(1), [ '~/data/h7_placeMap_' dateStr  '_' strrep(ttFilenames{ttIdx},'.NTT','') '_cluster_' num2str(whichCell) '_.png'],'-dpng','-r200');
            clf(gcf(1));
