%% load data
dir='/Volumes/Seagate Expansion Drive/h5/2018-04-10/part3_trad/';
[ ~, spiketimes, spikeheader, ~, cellNumber ]=ntt2mat([ dir 'TT10_cuts.NTT' ]);
...
cellXposAll = zeros(size(spiketimes));
cellYposAll = cellXposAll;
cellSpeedAll = cellXposAll;
for ii=1:length(spiketimes)
    tempIdx=find(spiketimes(ii)<xytimestamps, 1 );
        if ~isempty(tempIdx)
            cellXposAll(ii)=xpos(tempIdx);
            cellYposAll(ii)=ypos(tempIdx);
            cellSpeedAll(ii) = speed(tempIdx);
        end
end
% simple speed filter hack
speedThreshold = 9; % cm/s
cellXposAll=cellXposAll(cellSpeedAll>speedThreshold);
cellYposAll=cellYposAll(cellSpeedAll>speedThreshold);
cellN=cellNumber(cellSpeedAll>speedThreshold);
xyHist=twoDHistogram(xpos(speed>speedThreshold),ypos(speed>speedThreshold),10,720,420);
ppxy = xyHist/sum(xyHist(:)); % probability of being in place
% 
figure; 
for cellId=1:max(cellN); 
    subplot(3,5,cellId); 
    qq=twoDHistogram(cellXpos(cellN==cellId),cellYpos(cellN==cellId),10,720,420); 
    ppspike = qq./(xyHist./30); % spike rate
...
%
% STOPPED EDITING THIS COPY OF A THE FILE HERE ON WED APRIL 11, 2018
%
%
...
figure; subplot(2,3,1);  plot( xpos, ypos, 'Color', [.1 .1 .1 .1] ); subplot(2,3,2); plot( xpos, ypos, 'Color', [.1 .1 .1 .1] ); hold on; plot(cellXpos(cellNumber==4),cellYpos(cellNumber==4),'*');
subplot(2,3,4); xyHist=twoDHistogram(xpos,ypos,20,720,420); imagesc(flipud(xyHist)); colormap(build_NOAA_colorgradient);
...
figure; 
for cellId=1:max(cellNumber); 
    subplot(3,5,cellId); 
    qq=twoDHistogram(cellXpos(cellNumber==cellId),cellYpos(cellNumber==cellId),20); 
    imagesc(flipud(qq)); colormap(build_NOAA_colorgradient); end;
