...
metadata.dir=[ '/Volumes/Seagate Expansion Drive/plusmaze-habit/' metadata.rat '/' metadata.day '/'  ];
...
avgLfp=avgLfpFromList( metadata.dir, fileListGoodLfp, metadata.lfpStartIdx );  % build average LFP
avgDisconnectedLfp=avgLfpFromList( metadata.dir, fileListDisconnectedLfp, metadata.lfpStartIdx );  % build average LFP

% 
% 88, 61, 32 suspected SWR channel
[ lfp88, lfpTimestamps ] = csc2mat( [ metadata.dir 'CSC88.ncs' ], metadata.lfpStartIdx );
...
for ii = 1:length(chewDetectorOutput.EpisodePeakTimes)
    if ( ii > 1 ) && ( (chewDetectorOutput.EpisodePeakTimes(ii)-chewDetectorOutput.EpisodePeakTimes(ii-1)) < 20 )
        ...
        if ( relativeTimes(jj) > 0 )
            ...
            intoBucketTimes = [ intoBucketTimes electricSignal.PeakTimes(jj) ];
        else
            if ( jj < length(relativeTimes) )
                if ( relativeTimes(jj+1) < 40 )
                    % use this event
                    rewardTimes = [ rewardTimes chewDetectorOutput.EpisodePeakTimes(ii) ];
                    intoBucketTimes = [ intoBucketTimes electricSignal.PeakTimes(jj+1) ];
                else
...
if metadata.visualizeAll
    ...
    scatter( rewardTimes, zeros(size(rewardTimes)), 'o', 'filled', 'MarkerEdgeColor', [ .4 .4 .9 ], 'MarkerFaceColor', [ .4 .4 .9 ]);
    scatter( intoBucketTimes, zeros(size(intoBucketTimes)), '^', 'filled', 'MarkerEdgeColor', [ .9 .1 .1 ], 'MarkerFaceColor', [ .9 .1 .1 ]);
    ylabel('elec.');
    xlim([0 timestampSeconds(end)]);
...
if length(find(rewardTimes)) ~= length(find(intoBucketTimes))
...
halfWindowSize = round(10*sampleRate.lfp);  % elements
...
if ( median(mazeEntries.mvgMedian(1:20)) < (median(mazeEntries.mvgMedian)*2) )
    ontoMazeTimes(1) = mazeEntries.mvgMedianTimes(1); % this is a cheap hack
    startIdx=2;
else
    startIdx=1;
    disp('check ontoMaze times!');
end
for idxTrialEnd=startIdx:length(intoBucketTimes)
        if idxTrialEnd > 1
            relativeTimes = transMvgMedPeakTimes - intoBucketTimes(idxTrialEnd-1);
        else
...
if metadata.visualizeAll
   ...
   %
    ...
    scatter( rewardTimes, zeros(size(rewardTimes)), 'o', 'filled', 'MarkerEdgeColor', [ .4 .4 .9 ], 'MarkerFaceColor', [ .4 .4 .9 ]);
    scatter( intoBucketTimes, zeros(size(intoBucketTimes)), '^', 'filled', 'MarkerEdgeColor', [ .9 .1 .1 ], 'MarkerFaceColor', [ .9 .1 .1 ]);
    scatter( ontoMazeTimes, zeros(size(ontoMazeTimes)), 'v', 'MarkerEdgeColor', [ .5 .4 0 ], 'MarkerFaceColor', [ 1 .8 0 ] );
    ylabel('events');
    xlim([0 timestampSeconds(end)]);
    ylim([-0.001 max( [ max(electricSignal.PeakValues) max(chewDetectorOutput.EpisodePeakValues) ] ) ]);
end

if length(find(rewardTimes)) ~= length(find(ontoMazeTimes))
...
% * ontoMazeTimes need to be offset from intoBucket because one mounts
% maze, then enters bucket, onto maze, enter bucket...
for ii = 1:length(intoBucketTimes)-1
    ...
    startIdx = round((intoBucketTimes(ii)+1) * sampleRate.telemetry);
    endIdx   = round((ontoMazeTimes(ii+1)-3) * sampleRate.telemetry);
    if endIdx < startIdx
        startIdx = round((intoBucketTimes(ii)) * sampleRate.telemetry);
        endIdx   = round((ontoMazeTimes(ii+1)) * sampleRate.telemetry);
    end
...
if metadata.visualizeAll
    figure(2); % demonstrate position correction to check if it was appropriate
    subplot(3,1,1);
...
halfWindowSize = round(5*sampleRate.telemetry); % elements
...
plot( timestampSeconds, electricEnv, 'k' );      axis tight;  ylabel('Avg; 60 Hz');
scatter( electricSignal.PeakTimes, electricSignal.PeakValues, 'v', 'filled');
subplot(3,1,2); hold on;
plot( chewCrunchEnvTimes, chewEpisodeEnv, 'Color', [ .1 .8 .1 ] ); axis tight;  ylabel('reward/chew');
...
for ii=1:length(electricSignal.PeakTimes)
    idx = find(abs(swrPeakTimesDenoise-electricSignal.PeakTimes(ii))<1);
    swrPeakTimesDenoise(idx)=NaN;
    swrPeakValuesDenoise(idx)=NaN;
end
...
scatter( rewardTimes, ones(1,length(rewardTimes)).*0.1, 'o', 'filled', 'MarkerEdgeColor', [ .4 .4 .9 ], 'MarkerFaceColor', [ .4 .4 .9 ]);
scatter( intoBucketTimes, ones(1,length(intoBucketTimes)).*0.1, '^', 'filled', 'MarkerEdgeColor', [ .9 .1 .1 ], 'MarkerFaceColor', [ .9 .1 .1 ]);
ylabel( 'start prox./trial ev.' ); axis tight; ylim([ 0 1 ]);
subplot(3,1,2); 
histogram( swrPeakTimesDenoise, swrAnalysisBins ); 
title('SWR Frequency'); 
xlabel('time (s)'); ylabel('counts'); axis tight;
subplot(3,1,3); hold off;
xx=swrAnalysisBins(1:end-1)+diff(swrAnalysisBins)/2;
yy=histcounts( swrPeakTimesDenoise, swrAnalysisBins )./diff(swrAnalysisBins);
plot( xx, yy ); 
hold on;
title('SWR rate'); xlabel('time (s)'); ylabel('Hz'); axis tight;
[vals,idxs]=intersect(swrAnalysisBins,ontoMazeTimes);
scatter( xx(idxs), yy(idxs), 'v', 'MarkerEdgeColor', [ .5 .4 0 ], 'MarkerFaceColor', [ 1 .8 0 ] );;
[vals,idxs]=intersect(swrAnalysisBins,runStartTimes);
scatter( xx(idxs), yy(idxs), '>',  'MarkerEdgeColor', [ .2 .6 0 ], 'MarkerFaceColor', [ .2 .6 0 ]);
[vals,idxs]=intersect(swrAnalysisBins,rewardTimes);
scatter( xx(idxs), yy(idxs), 'o', 'filled', 'MarkerEdgeColor', [ .4 .4 .9 ], 'MarkerFaceColor', [ .4 .4 .9 ]);
[vals,idxs]=intersect(swrAnalysisBins,intoBucketTimes);
