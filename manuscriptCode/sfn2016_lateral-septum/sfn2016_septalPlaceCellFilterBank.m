%function [ phase, envelope, frequency, signal, timestamp, bpFiltered, bpEnveloped, bpPhase ] = lfpThetaPhaseFilterBank( lfpFilename, positionDataFilename, speedThreshold  )
...
powerThreshold = round(1e-6*60/bitvolts); % A-to-D Values  ( 60 microvolts / bitvolts )
...
thetaPhaseDegrees = zeros(numberOfBbandsToPass, length(decimatedLFP));
envelopeThetaLFP = zeros(numberOfBbandsToPass, length(decimatedLFP));
for ii = 1:numberOfBbandsToPass
    thetaPhaseDegrees(ii,:) = angle(hilbertLFP(ii,:))*180/pi;
    envelopeThetaLFP(ii,:) = abs(hilbertLFP(ii,:));
end

% there's probably more correct 2d way to implement this.
for ii = 1:length(envelopeThetaLFP(1,:))
    [mag,pos] = max(envelopeThetaLFP(:,ii));
    if ( mag < powerThreshold )
        phaseMaster(ii) = -1;
    else
        phaseMaster(ii) = thetaPhaseDegrees(pos,ii);
    end
    instantFrequency(ii)= bandpassCenterFrequencies(pos);
end


% so we need time to deal with some of this.
% the position and velocity, etc data is all at 30 fps which is to say 30
% Hz
...
% LFP data file.
freq30Hz = interp1( timestamps250, freq250, positionData.tpos )
phase30Hz = interp1( timestamps250, phase250, positionData.tpos )
[corr,lag]= xcorr(freq30,spd)


interp1(downsample(fulltimestamps, 128),)
