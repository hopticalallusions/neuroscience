...
% way?
figure;
subplot(2,2,1); plot(xpos(1:round(end*1/4)),ypos(1:round(end*1/4))); axis([ 0 720 0 480 ]); %legend('1st Qtr');
subplot(2,2,2); plot(xpos(round(end*1/4):round(end*2/4)),ypos(round(end*1/4):round(end*2/4))); axis([ 0 720 0 480 ]); %legend('2nd Qtr');
subplot(2,2,3); plot(xpos(round(end*2/4):round(end*3/4)),ypos(round(end*2/4):round(end*3/4))); axis([ 0 720 0 480 ]); %legend('3rd Qtr');
...
figure;
subplot(2,1,1);hold on; plot((tempDaIdxs)/600, (daConc(tempDaIdxs)-min(daConc(tempDaIdxs)))/(max(daConc(tempDaIdxs)-min(daConc(tempDaIdxs))))); 
...
subplot(2,1,1); plot(tempTimePrime, (tempVelocity-min(tempVelocity))/max(tempVelocity-min(tempVelocity)) ); legend('\Delta DA (nM)', 'velocity'); axis tight;
subplot(2,1,2); plot((alignmentLag/600)+tempNlxIdxs/(60*30), distFromPoint( xpos(tempNlxIdxs), ypos(tempNlxIdxs), 367, 121 )); %legend('dist from lower');
...
[corr,lag]=xcorr(daConc,[zeros(alignmentLag,1); decimate(distFromPoint( xpos, ypos, 367, 121 ),3)],80);
plot(lag/10,corr/(max(corr)-min(corr)));
