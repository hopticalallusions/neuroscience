...
To verify the conclusion in the previous paragraph, the data is displayed above as a QQ plot. As expected, the data do not conform well to Gaussian distributions.
...
#### Alternate Version with Dual Box Bootstrap (assumption of difference)
#### Part II.C : Bootstrapping the Anatomical Location Data
##### Dual Box Bootstrap Difference the Ratios
...
Here we have assumed that the data sets were drawn from different populations; the null hypothesis is that despite the difference in population, the data sets have the same statistic. Here, the statistic is difference. The plot above shows 10,000 simulated datasets where and the resulting simulated differences. The gray bars represent the region where 95% of the differences lie (not consisdered significantly different from the null hypothesis). The green and cyan regions combined represent 5% of the data while the cyan region represent 1% of differences. The actual observed difference (effect size) of dab-1 knockout ratios minus wildtype ratios is  `r round(median(scmMedLatRatios) - median(wtMedLatRatios), 3)`, which is actually expected in approximately `r round( which(sort(c(actualMedianDiff, diffBootyData))==actualMedianDiff )/length(c(actualMedianDiff, diffBootyData)) *100,2)`% of cases where the null hypothesis is true. In other words, it is highly unlikely that these populations are the same, suggesting that dab-1 knockout have a significant effect on lateral to medial neuronal migration.

#####Alternate Version : Dual Box Bootstrap Ratio the Ratios
...
#### Alternate Version with Dual Box Bootstrap (assumption of difference)
#### Part II.E : Bootstrapping the Anatomical Location Data
##### Dual Box Bootstrap Difference the Differences

```{r, echo=FALSE}

# use big booty if the thought is that all the data came from the same underlying distribution

diffBootyData <- dualBooty( wtMedLatDiff, scmMedLatDiff, 10000, "minus" )
...
plot(tempHist,col=c(rep("cyan",cyanRealLow), rep("green",greenLow), rep("lightgrey",redCount), rep("green", greenLow), rep("cyan",cyanRealHigh)), xlab='Difference', main='Bootstrapped Significance of \nObserved Median Difference Difference', las=1)
actualMedianDiff = median(scmMedLatDiff) - median(wtMedLatDiff)
lines(c(actualMedianDiff, actualMedianDiff),c(0, quantilefx(tempHist$counts,65)), lwd=2)
#points( c(actualMedianDiff,actualMedianDiff), c(quantilefx(tempHist$counts,827), quantilefx(tempHist$counts,87)*1.04), pch=c('x', '~')) # good grief. setting up LaTeX was taking too long
#
...
Here we have assumed that the data sets were drawn from different populations; the null hypothesis is that despite the difference in population, the data sets have the same statistic. Here, the statistic is difference. The plot above shows 10,000 simulated datasets where and the resulting simulated differences. The gray bars represent the region where 95% of the differences lie (not consisdered significantly different from the null hypothesis). The green and cyan regions combined represent 5% of the data while the cyan region represent 1% of differences. The actual observed difference (effect size) of dab-1 knockout ratios minus wildtype ratios is  `r round(median(scmMedLatDiff) - median(wtMedLatDiff), 3)`, which is actually expected in approximately `r round( which(sort(c(actualMedianDiff, diffBootyData))==actualMedianDiff )/length(c(actualMedianDiff, diffBootyData)) *100,2)`% of cases where the null hypothesis is true. In other words, it is highly unlikely that these populations are the same, suggesting that dab-1 knockout have a significant effect on lateral to medial neuronal migration.


#####Alternate Version : Dual Box Bootstrap Ratio the Differences

```{r echo=FALSE}

### check if this actually works correctly.~~!!!

ratioBootyData <- dualBooty( wtMedLatDiff, scmMedLatDiff, 10000, "ratio" )

tempHist <- hist(ratioBootyData, breaks=50)
