...
disp(['Filtering waveforms f=' num2str(f_low) '-' num2str(f_high) ' Hz. '])
...
for i=1:length(dounits);
    ...
    save([finalwavedir 'waveforms_i' num2str(wavefilecounter(uniti)) '_cl' num2str(uniti) '.mat'],'waveforms', '-mat');  %saves empty file.   
end
...
disp(['trial ' num2str(trial) ' of ' num2str(numberof_wavetrials)])
    ...
    for unitind=1:length(dounits);
        ...
        if length(pointsintrial)>plotmax
            pointsintrial=pointsintrial(1:plotmax);
            timesintrial=timesintrial(1:plotmax);
            jittersintrial=jittersintrial(1:plotmax);      
        elseif length(pointsintrial)==0   
            continue   %skip to next unit
        end
         
        wavefile=dir([finalwavedir 'waveforms_i' num2str(wavefilecounter(unit)) '_cl' num2str(unit) '.mat']);   %load & initialize waveform file.   
        if wavefile.bytes<maxwavesize;                    
        load([finalwavedir 'waveforms_i' num2str(wavefilecounter(unit)) '_cl' num2str(unit) '.mat']);          
        else                       
        ...
        save([finalwavedir 'waveforms_i' num2str(wavefilecounter(unit)) '_cl' num2str(unit) '.mat'],'waveforms', '-mat')
            
        end 
           
        if length(find(stimes<(trial*trialduration)))>=plotmax   %added November 7 2011; if 'n' then in the next trial won't append any more waves for this uniter.
%         disp(['stopped appending waves for unit ' num2str(unit) ', because already added at least ' num2str(plotmax) ' spikes.'])
        appendmorespikes{unit}='n';       %note: plotmax is specified in set_plot_parameters
        end
